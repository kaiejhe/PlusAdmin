                  ...
                  <CardContent>
                    <form class="space-y-4" @submit.prevent="submitForm">
                      ...
                      >
                        ...
                        <div
                          v-if="config.table === 'TeamToken' && field.key === 'AccToken' && teamTokenPreview?.error"
                          class="rounded-md bg-gray-50 px-3 py-2 text-xs text-red-600"
                        >
...
function parseTeamTokenInput(rawInput, baseEmail = '', baseTeamId = '', options = {}) {
  ...
  if (trimmed.startsWith('{')) {
    ...
    try {
      ...
      throw new Error('团队密钥格式错误，请粘贴完整 JSON 文本');
    }
...
const teamTokenAutoFillState = reactive({
  enabled: false,
  lastEmail: '',
  lastTeamId: '',
});

function resetTeamTokenAutoFillSnapshot() {
  teamTokenAutoFillState.lastEmail = '';
  teamTokenAutoFillState.lastTeamId = '';
}

function shouldAutoFillField(currentValue, lastAutoValue) {
...
watch(
  () => [showFormModal.value, isTeamTokenResource.value, isEditing.value],
  ([visible, isTeamToken, editing]) => {
    const shouldEnable = Boolean(visible && isTeamToken && !editing);
    if (teamTokenAutoFillState.enabled !== shouldEnable) {
      teamTokenAutoFillState.enabled = shouldEnable;
      resetTeamTokenAutoFillSnapshot();
    }
  },
  { immediate: true },
);
...
watch(
  () => formModel.AccToken,
  (rawInput) => {
    if (!teamTokenAutoFillState.enabled) return;
    const parsed = parseTeamTokenInput(
      rawInput,
      formModel.TeamEmail,
      formModel.TeamID,
      { silent: true },
    ...
    if (!parsed) return;
    const nextEmail = (parsed.teamEmail ?? '').trim();
    if (
      nextEmail &&
      shouldAutoFillField(formModel.TeamEmail, teamTokenAutoFillState.lastEmail)
    ) {
      formModel.TeamEmail = nextEmail;
      teamTokenAutoFillState.lastEmail = nextEmail;
    }
    const nextTeamId = (parsed.teamId ?? '').trim();
    if (
      nextTeamId &&
      shouldAutoFillField(formModel.TeamID, teamTokenAutoFillState.lastTeamId)
    ) {
      formModel.TeamID = nextTeamId;
      teamTokenAutoFillState.lastTeamId = nextTeamId;
    }
  },
);
...
async function applyResourceSpecificTransforms(payload) {
  ...
  if (table === 'TeamToken') {
    ...
    const baseEmail = (formModel.TeamEmail ?? '').trim() || editingRecord.value?.TeamEmail || '';
    const baseTeamId = (formModel.TeamID ?? '').trim() || editingRecord.value?.TeamID || '';
    const parsed = parseTeamTokenInput(rawInput, baseEmail, baseTeamId, { requireValue: true });
    const accessToken = parsed?.accessToken ?? '';
    const teamEmail = parsed?.teamEmail ?? '';
    const teamId = parsed?.teamId ?? '';
    if (!accessToken) {
      throw new Error('无法获取 accessToken，请确认密钥内容有效');
    }
    if (!teamEmail || !teamId) {
      throw new Error('无法解析团队邮箱或团队编号，请确认 JSON 内容准确');
    }
