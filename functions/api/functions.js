...
export async function TeamEmail(request, env){
  ...
  if(res.ok && result.status==='success'){
    try {
      ...
      return json({ ok: false, msg: "服务器异常,请重试或联系客服处理",
        TeamRES:TeamRES.accEmail,chinaTime:chinaTime,CardRes:CardRes.CardTime,
        result:{ message: error?.message || String(error), stack: error?.stack },}, 200); 
    }
  }else{
